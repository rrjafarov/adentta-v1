...
// !   bu kof claudun structor deyisiminden evvelki versiyasidir.   ! \\
"use client";
import Link from "next/link";
import React, {
  useState,
...
function collectDescendants(childrenMap, rootIds = []) {
  ...
  for (const r of rootIds) {
    const rid = Number(r);
    if (Number.isFinite(rid)) stack.push(rid);
  }
...
export default function ProductsPageFilter({
  ...
  // ---------- NEW: URL → STATE SYNC ----------
  // URL-də category/brands dəyişəndə state-i yenilə ki, sorğular doğru id-lərlə getsin
  useEffect(() => {
    // category: slugs -> ids
    const catParam = searchParams.get("category") || "";
    const slugList = catParam ? catParam.split(",").filter(Boolean) : [];
    const idsFromUrl = slugList
      .map((slug) => {
        const c = categoryData.find((cc) => cc.url_slug === slug);
        if (!c) return null;
    ...
    setSelectedCategoryIds((prev) => {
      const prevKey = normalizeIds(prev).join(",");
      const nextKey = normalizeIds(idsFromUrl).join(",");
      return prevKey === nextKey ? prev : normalizeIds(idsFromUrl);
    });

    // brands: "1,2,3" -> [1,2,3]
    const brandParam = searchParams.get("brands") || "";
    const brandIdsFromUrl = brandParam
      ? brandParam
          .split(",")
          .map((s) => parseInt(s, 10))
    ...
    setSelectedBrandIds((prev) => {
      const prevKey = normalizeIds(prev).join(",");
      const nextKey = normalizeIds(brandIdsFromUrl).join(",");
      return prevKey === nextKey ? prev : normalizeIds(brandIdsFromUrl);
    });
  }, [searchParams.toString(), categoryData]);

  // ---------- category children map ----------
  const childrenMap = useMemo(() => buildChildrenMap(categoryData), [categoryData]);

  const expandWithDescendants = useCallback((ids = []) => {
    const base = normalizeIds(ids);
    if (base.length === 0) return [];
    const all = new Set();
    for (const id of base) {
      all.add(Number(id)); // root özü
      const kids = collectDescendants(childrenMap, [id]);
      for (const k of kids) all.add(Number(k));
    }
  ...
  const fetchProductsDebounced = useCallback(
    async (categoryIds, brandIds, searchTextParam, page = 1) => {
      ...
      debounceTimeoutRef.current = setTimeout(async () => {
        setIsLoading(true);
        try {
          // parent seçilibsə bütün altlarla genişləndir
          const effectiveCategoryIds = expandWithDescendants(categoryIds);
          const { products, pagination: newPagination } = await fetchProducts(
  ...
  const fetchMoreProducts = async (page) => {
    ...
    try {
      ...
      const searchTextFromParams =
        paramsObj.search_text || searchParams.get("search_text") || "";

      let queryString = "";

      // parent seçilibsə bütün altlarla genişləndir
      const effectiveCategoryIds = expandWithDescendants(selectedCategoryIds);
      ...
      if (effectiveCategoryIds.length > 0) {
        ...
        if (searchTextFromParams) {
          queryString += `&search_text=${encodeURIComponent(
            String(searchTextFromParams)
          )}`;
        }
      } else {
        // default davranış (kateqoriya seçilməyibsə)
        paramsObj.page = page;
        ...
        if (selectedBrandIds.length > 0 && !paramsObj["filters[1][key]"]) {
          ...
          selectedBrandIds.forEach((id) =>
            paramsObj["filters[1][value][]"]
              ? paramsObj["filters[1][value][]"].push(String(id))
              : (paramsObj["filters[1][value][]"] = [String(id)])
          );
        }
  ...
  useEffect(() => {
    ...
    prevParamsRef.current = currentParamsStr;
  }, [searchParams.toString(), selectedCategoryIds, selectedBrandIds, expandWithDescendants, perPage]);
  ...
  const handleCategoryToggleById = useCallback(
    (id) => {
      ...
      setSelectedCategoryIds((prev) => {
        ...
        // Immediate URL update for better UX (URL-da yalnız seçilən root-lar yazılır)
  ...
  // --------- HEADER DROPDOWN MENTIQI ----------
  // Normalizasiya (Header-dən gələn categoryData-nı qruplama)
    ...
    <div>
      <div className="container">
          ...
          <div className="xl-3 lg-3 md-3 sm-12">
            <div className="filter-container">
              ...
              <div className={`filter-panel ${isMobileFilterOpen ? "active" : ""}`}>
                ...
                {/* Categories - Header Dropdown Struktur */}
                <FilterAccordion title={t?.productsPageFilterCategoryTitle || "Category"}>
                  <ul style={{ maxHeight: "300px", overflowY: "auto", paddingRight: "4px" }}>
                    {roots.length === 0 ? (
                      <li>{t?.noCategories || "No categories"}</li>
                    ) : (
                      roots.map((rootCategory) => {
                        const children = getChildrenForRoot(rootCategory.id);
                        const parentProductCount = getProductCountForCategory(rootCategory.id);
                        const isParentSelected = selectedCategoryIds.some((c) => Number(c) === Number(rootCategory.id));

                        return (
                          <React.Fragment key={rootCategory.id}>
                            <li
                              onClick={() => handleCategoryToggleById(rootCategory.id)}
                              style={{
                                cursor: "pointer",
                                display: "flex",
                                alignItems: "center",
        ...
        <style jsx>{`
          ...
          .productsPageDetailsCEO h1 { margin-bottom: 0.5rem; }
          .page-description-content { margin-bottom: 0.5rem; }
        `}</style>
      </div>
    </div>
  );
...
// !Burda mehsullar tam gelir alt ust kategory
// "use client";
// import Link from "next/link";
// import React, {
//   useState,
...
//   // ---------- NEW: category children map və expand helper-ları (memoized) ----------
//   const childrenMap = useMemo(() => buildChildrenMap(categoryData), [categoryData]);
...
//         setIsLoading(true);
//         try {
//           // NEW: parent seçilibsə bütün altlarla genişləndir
//           const effectiveCategoryIds = expandWithDescendants(categoryIds);
//           const { products, pagination: newPagination } = await fetchProducts(
//             effectiveCategoryIds,
//             brandIds,
...
//         // FIX: search_text həmişə qorunsun (kateqoriya seçiləndə də)
//         if (searchTextFromParams) {
//           queryString += `&search_text=${encodeURIComponent(
//             String(searchTextFromParams)
//           )}`;
//         }
//       } else {
//         // FIX: default olaraq SAHTƏ kateqoriya 99 əlavə ETMƏ!
//         paramsObj.page = page;
//         paramsObj.per_page = perPageVal;
...
//           // FIX: search_text kateqoriya seçili olanda da əlavə olunsun
//           if (stParam) {
//             queryString += `&search_text=${encodeURIComponent(stParam)}`;
//           }
//         } else {
...
//           };
//           queryString = buildRawQuery(params);
//           // stParam artıq params içindədirsə təkrar əlavə etməyə ehtiyac yoxdur
//         }
...
// !    NERIMAN BEYIN KODU ASAGIDADIR    29.10
// "use client";
// import Link from "next/link";
// import React, {
//   useState,
