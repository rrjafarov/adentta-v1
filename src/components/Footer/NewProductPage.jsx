...
const ProductsPageFilter = ({
  ...
  const handleCategoryToggleById = useCallback(
    (id) => {
      ...
      setSelectedCategoryIds((prev) => {
        ...
        // router.push ilə dərhal update + server-component refresh. 😀
        router.push(pathname + (newSearch ? `?${newSearch}` : ""), {
          scroll: false,
        });
        router.refresh(); // 😀 force server components to re-run so Hero updates
  ...
  const handleBrandToggleById = useCallback(
    (brandId) => {
      ...
      router.refresh(); // 😀 ensure server components (Hero) re-run
    },
    [router, searchParams]
  );
  ...
  const fetchMoreProducts = async (page) => {
    ...
    try {
      ...
      // removed artificial delay (was await delay(1000))
      ...
      if (newItems.length > 0) {
        ...
        // Use router.push for page change (no forced refresh here) 😀
