...
const ProductsPageFilter = ({
  ...
  // URL-dən seçilmiş kateqoriya title-ini götürmək üçün funksiya
  const getSelectedCategoryTitle = () => {
    // URL-dən filters[0][value][] parametrini götür
    const categoryId = searchParams.get("filters[0][value][]");
    
    if (!categoryId || !productData || productData.length === 0) {
      return null;
    }
    
    // productData-dan kateqoriya ID-nə uyğun olan kateqoriyanı tap
  ...
  // Kateqoriya silmək funksiyası
  const handleRemoveCategory = () => {
    const params = new URLSearchParams(searchParams.toString());
    // filters[0][value][] parametrini sil
    params.delete("filters[0][key]");
  ...
  /* -----------------------------
     ...
     - buildCategoryTree handles:
       * already nested categories (children arrays)
       * flat array with parent_id or parent.id fields
  ------------------------------*/
  const buildCategoryTree = (rawCategories) => {
    if (!Array.isArray(rawCategories)) return [];

    // if already nested (children present on items), just return roots
    const hasChildrenField = rawCategories.some((c) => Array.isArray(c.children) && c.children.length > 0);
    if (hasChildrenField) {
      // filter root nodes (those without parent_id / parent)
      return rawCategories.filter((c) => !c.parent_id && !(c.parent && c.parent.id));
    }

    // else try to build tree from flat list using id / parent_id / parent.id
    const map = {};
    rawCategories.forEach((c) => {
      const id = c.id ?? c._id ?? c.value ?? null;
      if (id == null) return;
      map[id] = { ...c, children: [] };
    });
    ...
    rawCategories.forEach((c) => {
      const id = c.id ?? c._id ?? c.value ?? null;
      if (id == null) return;
      const parentId = c.parent_id ?? (c.parent && (c.parent.id ?? c.parent)) ?? null;
  ...
  const countProductsForCategory = (catId) => {
    if (!productData || !Array.isArray(productData)) return 0;
    return productData.filter((p) =>
      Array.isArray(p.categories) && p.categories.some((c) => String(c.id) === String(catId))
    ).length;
  };

  const renderCategoryItem = (cat) => {
    const count = countProductsForCategory(cat.id ?? cat._id ?? cat.value ?? "");
    return (
      <li key={cat.id ?? cat._id ?? cat.value} className="categoryPortal">
        <div className="categoryRow">
          <span className="catTitle">{cat.title || cat.name}</span>
          <p className="catCount">({count})</p>
        </div>

        {Array.isArray(cat.children) && cat.children.length > 0 && (
          <ul className="subCategoryPortal">
            {cat.children.map((sub) => (
              <li key={sub.id ?? sub._id ?? sub.value} className="subCategoryItem">
                <div className="categoryRow">
                  <span className="catTitle">{sub.title || sub.name}</span>
                  <p className="catCount">({countProductsForCategory(sub.id ?? sub._id ?? sub.value)})</p>
                </div>
              </li>
            ))}
          </ul>
  ...
  return (
    <div>
        ...
        <div className="row">
          <div className="xl-3 lg-3 md-3 sm-12">
            <div className="filter-container">
              ...
              {/* Desktop için seçili filtreler */}
              {selectedCategoryTitle && (
                <div className="selectedFilter desktop-only">
                  <div className="selectedFilterInner">
                    <span onClick={handleRemoveCategory}>x</span>
              ...
              >
                ...
                >
                  <ul>
                    {/* Eğer categoryData varsa ondan render et, yoxsa fallback static list */}
                    {Array.isArray(categoryTree) && categoryTree.length > 0 ? (
                      categoryTree.map((cat) => renderCategoryItem(cat))
                    ) : (
                      <>
                        <li className="categoryPortal">
                          X-ray Equipment<p>(22)</p>
                          <ul className="subCategoryPortal">
                            <li>test </li>
                          </ul>
      ...
      <style jsx>{`
        ...
        /* Yeni əlavə olunan kateqoriya/sub-kateqoriya styling */
        .categoryPortal {
        ...
        .subCategoryPortal {
          margin-left: 1.5rem; /* Sən istədiyin indent */
          margin-top: 0.4rem;
          list-style: none;
          padding-left: 0;
        }
